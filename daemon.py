...
def get_speed(flag, speed_up, speed_down):
    while True:
        try:
            host = "localhost"
            port = 1338
                ...
                with conn:
                    while True:
                        try:
                            ...
                            sp_up = str(speed_up.value)
                            sp_down = str(speed_down.value)
                            sp = sp_up + " " + sp_down

                            conn.sendall(sp.encode())
                        except Exception:
                            pass
                        if (flag.value == 2):
                            break
...
if __name__ == "__main__":
    ...
    flag = Value('i', 0)
    window_size = Value('i', 64)
    speed_up = Value('f', 0)
    speed_down = Value('f', 0)
    lock = RLock()
    ...
    parser_fetchall(name_up, priority_up, server_up, name_down, priority_down, server_down)

    '''load json'''
    for i in range(len(server_up)):
        with open(server_up[i], 'r') as fd:
            server_up[i] = json.load(fd)

        with open(server_up[i]['password'], 'r') as fd:
            server_up[i]['password'] = fd.readline()[:-1]

        server_up[i]['key'] = paramiko.RSAKey.from_private_key_file(server_up[i]['key'], server_up[i]['password'])
    
    for i in range(len(server_down)):
        with open(server_down[i], 'r') as fd:
            server_down[i] = json.load(fd)

        with open(server_down[i]['password'], 'r') as fd:
            server_down[i]['password'] = fd.readline()[:-1]

        server_down[i]['key'] = paramiko.RSAKey.from_private_key_file(server_down[i]['key'], server_down[i]['password'])
    '''end'''

    proc = Process(target=file_transport, \
        args=(name_up, priority_up, server_up, name_down, priority_down, server_down, flag, lock, window_size, \
              speed_up, speed_down))

    proc.start()

    listen_proc = Process(target=listen_stop, args=(flag, lock))

    listen_proc.start()

    speed_proc = Process(target=get_speed, args=(flag, speed_up, speed_down))
    ...
    while True:
        ...
        if not proc.is_alive():
            window_size.value = window_size.value // 2
            window_size.value = max(window_size.value, 1)
